# DGB Sentinel AI Shield v4 Real Crypto Backend Contract
...
V4.8F-C introduces a deployment-controlled real ML-DSA adapter path for DGB Sentinel AI. It does not replace the deterministic TEST-ONLY signature path used by contract tests. It does not make DGB Sentinel AI a transaction signer, broadcaster, consensus layer, wallet custody layer, or AdamantineOS final authority.

## Non-authority lock

DGB Sentinel AI Shield v4 cryptography proves DGB Sentinel AI component decision evidence only.

DGB Sentinel AI still must not:
...
The Shield Orchestrator verifies DGB Sentinel AI component evidence before producing a Shield receipt. AdamantineOS remains the final execution boundary.
...
DGB Sentinel AI exposes a backend-neutral adapter contract in:
...
It lazily imports `oqs` only when used, so normal CI and non-OQS deployments do not silently depend on local machine crypto state. If OQS is missing, disabled, lacks the locked mechanism, or raises a native backend exception, the adapter wraps that failure inside the DGB Sentinel AI real-backend fail-closed error hierarchy.
...
Every real DGB Sentinel AI component-verdict signature signs the exact byte string:
...
- `algorithm`, `key_id`, and `key_version` must match the DGB Sentinel AI trust-profile entry.

The `signed_payload_hash` is already computed over the domain-separated canonical DGB Sentinel AI verdict payload. The real-signature input binds that hash to the concrete signature entry so signatures cannot be spliced across algorithms, keys, roles, or bundles.

## Binary encoding lock

Real ML-DSA signatures and public keys are binary. DGB Sentinel AI real backend adapters use explicit unpadded base64url encoding with the prefix:
...
- real OQS public keys use `b64u:` in the DGB Sentinel AI trust profile;
...
- structurally valid base64url that decodes to backend-invalid key or signature lengths must fail closed through `SentinelAiV4RealCryptoBackendError`;
...
The DGB Sentinel AI real-backend boundary wraps failures from:
...
Missing OQS and disabled OQS mechanisms surface through `SentinelAiV4RealCryptoBackendUnavailable`. All other backend failures surface through `SentinelAiV4RealCryptoBackendError`, with the native exception preserved as `__cause__`. Signature entries and registry key records must also match their exact expected field sets; extra authority-like fields fail closed.
...
A production real-backend deployment must satisfy both required paths. This DGB Sentinel AI OQS adapter alone does not downgrade policy.v1 and does not allow ML-DSA to replace the required classical path.
