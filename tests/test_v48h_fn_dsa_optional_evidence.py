...
from sentinel_ai_v2.v4.crypto_verdict import validate_crypto_verdict_envelope
...
def _tampered_signature_material(
    *,
    ...
    return hashlib.sha256(
        (
            "TEST-ONLY-SENTINEL-AI-SIGNATURE\n"
            f"{public_key}\n"
            f"{algorithm}\n"
            f"{standard_profile}\n"
            f"{signed_hash}"
...
def test_v48h_sentinel_ai_fn_dsa_absent_allowed_and_valid_optional_evidence_recorded() -> None:
...
def test_v49i4_sentinel_ai_bundle_builder_emits_canonical_order_without_mutating_input(
    ...
    assert [entry["algorithm"] for entry in bundle["signatures"]] == expected_algorithms
    assert supplied_signatures == original_signatures
    assert bundle["signatures"] is not supplied_signatures

    supplied_signatures.reverse()
    assert [entry["algorithm"] for entry in bundle["signatures"]] == expected_algorithms


def test_v49i4_sentinel_ai_bundle_builder_preserves_empty_internal_shell_construction() -> None:
    bundle = build_signature_bundle(signatures=[])

    assert bundle["signatures"] == []


def test_v49i4_sentinel_ai_bundle_builder_rejects_non_object_entry() -> None:
...
@pytest.mark.parametrize("entry", [{}, {"algorithm": "unknown"}])
def test_v49i4_sentinel_ai_bundle_builder_rejects_missing_or_unknown_algorithm(entry: dict) -> None:
    with pytest.raises(ValueError):
        build_signature_bundle(signatures=[entry])
...
def test_v49i4_sentinel_ai_verifier_rejects_noncanonical_order_before_key_lookup_or_crypto(
    ...
    monkeypatch.setattr(
        "sentinel_ai_v2.v4.signing.find_trusted_key",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (ML_DSA,),
        (FN_DSA,),
        (CLASSICAL_ED25519, FN_DSA),
        (ML_DSA, FN_DSA),
    ],
)
def test_v49i4_sentinel_ai_verifier_rejects_missing_required_before_key_lookup_or_crypto(
    algorithm_sequence: tuple[str, ...],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload_hash = signed_payload_hash(payload=unsigned_payload())
    ...
    def tracking_find_trusted_key(*args: object, **kwargs: object) -> dict:
        key_lookup_calls.append((args, kwargs))
        raise AssertionError("missing-required failure reached key lookup")
...
def test_v49i4_sentinel_ai_verifier_completes_structural_preflight_before_key_lookup_or_crypto(
    defect: str,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload_hash = signed_payload_hash(payload=unsigned_payload())
    signatures = [
        build_test_signature_entry(algorithm=algorithm, signed_hash=payload_hash)
        for algorithm in (CLASSICAL_ED25519, ML_DSA, FN_DSA)
    ]
    bundle = build_signature_bundle(signatures=signatures)
    prepared_signatures = bundle["signatures"]
    if defect == "duplicate":
        prepared_signatures.append(copy.deepcopy(prepared_signatures[-1]))
    elif defect == "empty_key_id":
        prepared_signatures[-1]["key_id"] = ""
    elif defect == "extra_field":
        prepared_signatures[-1]["extra"] = "forbidden"
    elif defect == "invalid_key_version":
        prepared_signatures[-1]["key_version"] = 0
    elif defect == "missing_field":
        del prepared_signatures[-1]["signature"]
    elif defect == "non_object":
        prepared_signatures.append("bad")  # type: ignore[arg-type]
    elif defect == "unknown_algorithm":
        prepared_signatures[-1]["algorithm"] = "unknown"
    elif defect == "wrong_domain":
        prepared_signatures[-1]["domain_tag"] = "wrong-domain"
    elif defect == "wrong_hash":
        prepared_signatures[-1]["signed_payload_hash"] = "b" * 64
    elif defect == "wrong_profile":
        prepared_signatures[-1]["standard_profile"] = "wrong-profile"
    ...
    def tracking_find_trusted_key(*args: object, **kwargs: object) -> dict:
        key_lookup_calls.append((args, kwargs))
        raise AssertionError("structural failure reached key lookup")
...
def test_v48h_sentinel_ai_valid_fn_dsa_cannot_rescue_required_failure(required_algorithm: str) -> None:
...
def test_v48h_sentinel_ai_fn_dsa_valid_cannot_replace_missing_ml_dsa() -> None:
...
def test_v48h_sentinel_ai_present_fn_dsa_invalid_duplicate_or_wrong_role_is_fatal() -> None:
    ...
    with pytest.raises(ValueError, match="trusted Sentinel AI key"):
...
def test_v48h_sentinel_ai_present_fn_dsa_no_registry_key_is_fatal_not_absent() -> None:
    verdict = _valid_bundle_verdict(algorithms=(CLASSICAL_ED25519, ML_DSA, FN_DSA))
    trust_profile = build_test_trust_profile()
...
def test_v48h_sentinel_ai_fn_dsa_profile_and_payload_binding_fail_closed() -> None:
...
def test_v48h_sentinel_ai_standard_profile_is_authenticated_not_metadata_only() -> None:
    verdict = _valid_bundle_verdict(algorithms=(CLASSICAL_ED25519, ML_DSA, FN_DSA))
...
def test_v48h_sentinel_ai_fn_dsa_bundle_verifier_records_profiles_order_stably() -> None:
    payload = unsigned_payload()
    payload_hash = signed_payload_hash(payload=payload)
...
def test_v48h_sentinel_ai_standard_profile_allow_list_rejects_empty_profile() -> None:
    with pytest.raises(ValueError, match="standard_profile"):
        require_supported_standard_profile(algorithm=FN_DSA, standard_profile="")


def test_v48h_sentinel_ai_signature_bundle_verifier_callback_fail_closed_contract() -> None:
    payload = unsigned_payload()
    payload_hash = signed_payload_hash(payload=payload)
    bundle = build_signature_bundle(
        signatures=[
