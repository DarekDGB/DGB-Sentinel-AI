...
class OqsFalcon1024Backend:
    ...
    def __init__(
        ...
        *,
        ...
        if mechanism != OQS_FALCON_MECHANISM:
            raise SentinelAiV4RealCryptoBackendError("Shield v4.8H requires OQS Falcon-1024 for draft FN-DSA evidence")
    ...
    def _load_oqs(self) -> Any:
        ...
        try:
            ...
            raise SentinelAiV4RealCryptoBackendUnavailable("liboqs-python import oqs is required for Falcon-1024") from exc
        except Exception as exc:
            self._raise_oqs_error("import", exc)

    def _raise_oqs_error(self, operation: str, exc: Exception) -> NoReturn:
        raise SentinelAiV4RealCryptoBackendError(f"OQS Falcon-1024 {operation} failed closed") from exc
    ...
    def _require_mechanism_enabled(self) -> Any:
        ...
        if self.mechanism not in enabled:
            raise SentinelAiV4RealCryptoBackendUnavailable("OQS Falcon-1024 mechanism is not enabled")
        return oqs
    ...
    def _require_expected_binary_length(
        ...
        *,
        ...
        if not isinstance(details, dict):
            raise SentinelAiV4RealCryptoBackendError("OQS Falcon-1024 details must be a mapping")
        expected = details.get(detail_key)
        if expected is None:
            return
        if isinstance(expected, bool) or not isinstance(expected, int) or expected <= 0:
            raise SentinelAiV4RealCryptoBackendError(f"OQS Falcon-1024 {detail_key} must be a positive integer")
        actual = len(value)
        if allow_shorter:
            if actual > expected:
                raise SentinelAiV4RealCryptoBackendError(f"{field} byte length must be <= {expected} for OQS Falcon-1024")
            return
        if actual != expected:
            raise SentinelAiV4RealCryptoBackendError(f"{field} byte length must be {expected} for OQS Falcon-1024")
    ...
    def sign_message(self, *, algorithm: str, private_key_reference: str, message: bytes) -> str:
        ...
        if algorithm != OQS_FALCON_ALGORITHM:
            raise SentinelAiV4RealCryptoBackendUnavailable("OQS Falcon backend only supports Shield v4 fn-dsa")
        ...
        *,
        ...
        try:
            ...
            if not isinstance(verified, bool):
                raise SentinelAiV4RealCryptoBackendError("OQS Falcon-1024 verify must return bool")
            return verified
