...
from sentinel_ai_v2.v4.signing import COMPONENT_VERDICT_DOMAIN
from sentinel_ai_v2.v4.trust_profile import require_non_empty_str, require_positive_int, require_supported_algorithm
...
class SentinelAiV4RealCryptoBackendError(ValueError):
    """Base fail-closed error for Sentinel AI Shield v4 real crypto backend wiring."""


class SentinelAiV4RealCryptoBackendUnavailable(SentinelAiV4RealCryptoBackendError):
    """Raised when a required production backend or algorithm is unavailable."""


class SentinelAiV4RealCryptoMaterialError(SentinelAiV4RealCryptoBackendError):
    """Raised when deterministic TEST-ONLY material reaches the real backend."""


class SentinelAiV4RealCryptoBackend(Protocol):
    """Minimal production crypto backend contract for Sentinel AI v4 evidence.
...
def _wrap_value_error(operation: str, callback: Callable[[], _T]) -> _T:
    try:
        ...
        raise SentinelAiV4RealCryptoBackendError(f"{operation} failed closed: {exc}") from exc


def _require_real_non_empty_str(value: Any, *, field: str) -> str:
    if not isinstance(value, str) or not value:
        raise SentinelAiV4RealCryptoBackendError(f"{field} must be non-empty string")
    if value != value.strip():
        raise SentinelAiV4RealCryptoBackendError(f"{field} must not contain surrounding whitespace")
    return value
...
def _require_hash(value: Any, *, field: str) -> str:
    ...
    if len(clean) != 64:
        raise SentinelAiV4RealCryptoBackendError(f"{field} must be 64-character sha256 hex")
    try:
        int(clean, 16)
    except ValueError as exc:
        raise SentinelAiV4RealCryptoBackendError(f"{field} must be sha256 hex") from exc
    if clean != clean.lower():
        raise SentinelAiV4RealCryptoBackendError(f"{field} must be lowercase sha256 hex")
    return clean
...
def _reject_test_only_text(value: str, *, field: str) -> None:
    ...
    if any(marker in clean for marker in _TEST_ONLY_MARKERS) or any(clean.startswith(prefix) for prefix in _TEST_ONLY_PREFIXES):
        raise SentinelAiV4RealCryptoMaterialError(f"{field} must not contain test-only material")
...
def encode_binary_signature_material(raw: bytes, *, field: str = "signature") -> str:
    ...
    if not isinstance(raw, bytes) or not raw:
        raise SentinelAiV4RealCryptoBackendError(f"{field} bytes must be non-empty")
...
def decode_binary_signature_material(encoded: Any, *, field: str = "signature") -> bytes:
    ...
    if not clean.startswith(REAL_SIGNATURE_ENCODING_PREFIX):
        raise SentinelAiV4RealCryptoBackendError(f"{field} must use b64u encoding")
    body = clean[len(REAL_SIGNATURE_ENCODING_PREFIX) :]
    if not body:
        raise SentinelAiV4RealCryptoBackendError(f"{field} b64u payload must be non-empty")
    if "=" in body:
        raise SentinelAiV4RealCryptoBackendError(f"{field} b64u payload must be unpadded")
    if set(body) - _BASE64URL_ALPHABET:
        raise SentinelAiV4RealCryptoBackendError(f"{field} b64u payload is invalid")
    try:
        decoded = base64.urlsafe_b64decode(body + "=" * (-len(body) % 4))
    except (binascii.Error, ValueError) as exc:
        raise SentinelAiV4RealCryptoBackendError(f"{field} b64u payload is invalid") from exc
    if not decoded:  # pragma: no cover - base64url cannot reach this after the body check.
        raise SentinelAiV4RealCryptoBackendError(f"{field} b64u payload must decode to non-empty bytes")
    return decoded
...
def build_real_crypto_signature_input(
    *,
    ...
    """Build the exact production-signature message bytes for Sentinel AI evidence.

    The signed payload hash is already domain-separated over the canonical Sentinel AI
    ...
    if clean_domain not in _ALLOWED_DOMAIN_TAGS:
        raise SentinelAiV4RealCryptoBackendError("domain_tag must be the Sentinel AI Shield v4 component signing domain")
...
def _require_backend_supports_algorithm(backend: SentinelAiV4RealCryptoBackend, algorithm: str) -> None:
    try:
        supported = tuple(getattr(backend, "supported_algorithms", ()))
    except Exception as exc:
        raise SentinelAiV4RealCryptoBackendError("real crypto backend algorithm discovery failed closed") from exc
    if algorithm not in supported:
        raise SentinelAiV4RealCryptoBackendUnavailable("real crypto backend does not support required algorithm")


def _call_backend_sign(
    backend: SentinelAiV4RealCryptoBackend,
    *,
    algorithm: str,
    ...
    except SentinelAiV4RealCryptoBackendError:
        raise
    except Exception as exc:
        raise SentinelAiV4RealCryptoBackendError("real crypto backend sign failed closed") from exc
...
def _call_backend_verify(
    ...
    *,
    ...
    try:
        ...
        raise SentinelAiV4RealCryptoBackendError("real crypto backend verify failed closed") from exc
    if not isinstance(verified, bool):
        raise SentinelAiV4RealCryptoBackendError("real crypto backend verify must return bool")
    return verified
...
def build_signature_entry_with_real_backend(
    *,
    ...
    """Build a Sentinel AI Shield v4 signature entry using a real backend."""
...
def _validated_key_fields(key: dict[str, Any], *, algorithm: str, key_id: str, key_version: int) -> dict[str, Any]:
    if not isinstance(key, dict):
        raise SentinelAiV4RealCryptoBackendError("registry key must be dict")
    if set(key.keys()) != _REGISTRY_KEY_FIELDS:
        raise SentinelAiV4RealCryptoBackendError("registry key fields must match required schema")
    role = _require_real_non_empty_str(key.get("role"), field="role")
    if role != COMPONENT_ROLE:
        raise SentinelAiV4RealCryptoBackendError("registry key role mismatch")
    ...
    if (key_algorithm, key_key_id, key_key_version) != (algorithm, key_id, key_version):
        raise SentinelAiV4RealCryptoBackendError("signature entry does not match registry key")
...
def verify_signature_entry_with_real_backend(
    ...
    *,
    ...
    """Verify one Sentinel AI Shield v4 signature entry with a production backend."""

    if not isinstance(entry, dict):
        raise SentinelAiV4RealCryptoBackendError("signature entry must be dict")
    if set(entry.keys()) != _SIGNATURE_ENTRY_FIELDS:
        raise SentinelAiV4RealCryptoBackendError("signature entry fields must match required schema")
...
def make_real_crypto_signature_verifier(
    ...
    """Adapt a real crypto backend to the existing Sentinel AI bundle verifier callback."""
